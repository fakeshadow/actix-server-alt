...
impl Client {
    ...
    /// // construct a new client and initialize connect request.
    /// let client = Client::new();
    /// let mut upgrade_request = client.upgrade("http://localhost:8080", Method::GET);
    /// upgrade_request.headers_mut().insert("upgrade", "protocol1, protocol2".parse()?);
    ///
    /// let mut upgrade_response = upgrade_request.send().await?;
    ///
